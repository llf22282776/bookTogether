...
export default class CoversionToPage extends Component {
    ...
    constructor(props){ 
        ...
        this.callback_operation=this.callback_operation.bind(this);
    }
            ...
            <View style={ styles.container }>
                ...
                <Left>
                 ...
                 <Button  transparent iconLeft  onPress={this.returnLastPage}>
                       <Icon name="md-arrow-back"/><Text style={ styles.bottomBtnText }>
                       返回</Text>
    ...
    btnRender(data,type){
        ...
        if(data.operation == "0" || data.operation == 0){
                //未确认，
            return  <CardItem>
                        <Button onPress={()=>this.operationFunc(0,type,data.mid)}><Text>确认</Text></Button>
                        <Button onPress={()=>this.operationFunc(1,type,data.mid)}><Text>拒绝</Text></Button>
                </CardItem>
    ...
    operationFunc(data,type,mid){
     
            //确认
              var parm={
                "rid":this.state.rid ,
                "sid":contant.USER.uname,
                "operation":data,
                "type":type,
                "mid":mid
         }
         contant.http_get(contant.SERVER_ROOT+contant.SERVER_SERVICE.UPDATE_RECORD_STATUS,parm,this.callback_operation,this.error_sendMsg);

        
    }
    callback_operation(data){
        //成功
        //需要刷新状态
        if(data.isSucceed=="true" || data.isSucceed == true){
              var parm={
    ...
    error_operation(data){
        //确认操作失败
        
        Alert.alert("错误","消息确认失败");
    }
